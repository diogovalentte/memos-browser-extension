...
import { getMemos, updateMemo } from '../../@/lib/actions/links.ts';
...
function findInSecondLine(list: { name: string, content: string }[], searchString: string): { name: string, content: string } | null {
  for (const memo of list) {
    const lines = memo.content.split('\n'); // Split content by newline
    
    // Check if the second line exists and contains the search string

    if (lines[1] && lines[1].includes(searchString)) {
      return { content: memo.content, name: memo.name }; // Return content and name
    }
  }
...
async function genericOnClick(
  ...
  if (!tab?.url || !tab?.title || !configured) {
    ...
    if (info.selectionText) {
        ...
        try {
            const memos = await getMemos(config.baseUrl, config.apiKey);
            const memo = findInSecondLine(memos, tab.url);
